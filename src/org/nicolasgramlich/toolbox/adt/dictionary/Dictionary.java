package org.nicolasgramlich.toolbox.adt.dictionary;
...
public class Dictionary {
	...
	private final BitVector mBitVector;
	...
	public Dictionary(final byte[] pBytes) {
		this(new BitVector(pBytes));
	}

	public Dictionary(final BitVector pBitVector) {
	...
	public static Dictionary load(final InputStream pInputStream) throws IOException {
		return new Dictionary(BitVector.load(new DataInputStream(pInputStream)));
	}
	...
	public static class Factory {
		...
		public static Dictionary create(final File pFile) throws IOException {
			return Dictionary.Factory.create(Util.readLines(pFile));
		}

		public static Dictionary create(final InputStream pInputStream) throws IOException {
			return Dictionary.Factory.create(Util.readLines(pInputStream));
		}

		public static Dictionary create(final Reader pReader) throws IOException {
			return Dictionary.Factory.create(Util.readLines(pReader));
		}
		...
		public static Dictionary create(final String ... pStrings) {
			...
			final int characterBitLength = Util.getBitLength(entryDictionary.mCharacters.length);
			final int entryCount = entryDictionary.mEntries.size();
			final int entryStartIndexBitLength = Util.getBitLength(entryDictionary.mEntryStartIndexMaximum);
			final int entryLengthBitLength = Util.getBitLength(entryDictionary.mEntryLengthMaximum);
			...
			final BitVector bitVector = new BitVector(bitsRequired);
		...
		public static class Entries {
			...
			public Entries(final ArrayList<Entry> pEntries, final String pString) {
				...
				final BitVector characterBitVector = new BitVector(Character.MAX_VALUE);
